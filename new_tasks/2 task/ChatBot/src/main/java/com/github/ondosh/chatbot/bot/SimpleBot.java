...
public class SimpleBot implements IBot {
    ...
    /**
     * Умножение словесной командой: «умножь 5 на 3»
     * Группа 1: первый операнд (5)
     * Группа 2: второй операнд (3)
     */
    private static final Pattern P_MUL  = Pattern.compile(
            "умножь\\s+(-?[\\d.,]+)\\s+на\\s+(-?[\\d.,]+)", Pattern.CASE_INSENSITIVE);

    /**
     * Деление словесной командой: «раздели 10 на 2»
     * Группа 1: делимое (10)
     * Группа 2: делитель (2)
     */
    ...
    /**
     * Сложение словесной командой: «сложи 4 и 6»
     * Группа 1: первое слагаемое (4)
     * Группа 2: второе слагаемое (6)
     */
    private static final Pattern P_ADD  = Pattern.compile(
            "сложи\\s+(-?[\\d.,]+)\\s+и\\s+(-?[\\d.,]+)", Pattern.CASE_INSENSITIVE);

    /**
     * Вычитание словесной командой: «вычти 3 из 10»
     * Группа 1: вычитаемое (3)
     * Группа 2: уменьшаемое (10)
     * Результат = 10 - 3
     */
    ...
    /**
     * Сложение в виде арифметического выражения: «5+3»
     * Группа 1: левый операнд (5)
     * Группа 2: правый операнд (3)
     */
    private static final Pattern P_EXPR_ADD = Pattern.compile(
            "(-?[\\d.,]+)\\s*\\+\\s*(-?[\\d.,]+)");

    /**
     * Вычитание в виде арифметического выражения: «8-2»
     * Группа 1: левый операнд (8)
     * Группа 2: правый операнд (2)
     * Результат = 8 - 2
     */
    ...
    /**
     * Умножение в виде арифметического выражения: «4*5» или «4×5»
     * Группа 1: левый операнд (4)
     * Группа 2: правый операнд (5)
     */
    private static final Pattern P_EXPR_MUL = Pattern.compile(
            "(-?[\\d.,]+)\\s*[*×]\\s*(-?[\\d.,]+)");

    /**
     * Деление в виде арифметического выражения: «15/3» или «15÷3»
     * Группа 1: делимое (15)
     * Группа 2: делитель (3)
     */
